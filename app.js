...
// --- SEMUA DATA (initialMasterData, initialHistoryData, globalDeletionHistory) ---
// --- SEKARANG DIMUAT DARI FILE 'app-data.js' ---


// =============================================
// BARU: FUNGSI LOCALSTORAGE
// =============================================
...
function saveDataToLocalStorage() {
    try {
        ...
        localStorage.setItem('n2store_app_data', JSON.stringify(dataToSave));
        console.log("Data berhasil disimpan ke localStorage.");
    } catch (error) {
        console.error("Gagal menyimpan data ke localStorage:", error);
        // Mungkin terjadi jika localStorage penuh
    }
}
...
function loadDataFromLocalStorage() {
    try {
        const savedDataString = localStorage.getItem('n2store_app_data');
        if (savedDataString) {
            const savedData = JSON.parse(savedDataString);
            
            // Timpa data default HANYA jika data tersimpan itu valid
            if (savedData.master) {
                initialMasterData = savedData.master;
            }
            if (savedData.history) {
                initialHistoryData = savedData.history;
            }
            if (savedData.trash) {
                globalDeletionHistory = savedData.trash;
            }
            console.log("Data berhasil dimuat dari localStorage.");
        } else {
            console.log("Tidak ada data di localStorage. Menggunakan data default.");
        }
    } catch (error) {
        console.error("Gagal memuat data dari localStorage:", error);
        // Jika JSON rusak, reset ke default
    }
}
 ...
 * Memformat angka menjadi string mata uang (Rp)
        ...
        function handleSimpanPenjualan() {
            ...
            try {
                ...
                if (sukses) {
                    ...
                    saveDataToLocalStorage(); // BARU: Simpan perubahan
                }
            } catch (error) {
        ...
        function importDataFromJSON(event) {
            ...
            reader.onload = function(e) {
                ...
                try {
                    ...
                    // BARU: Simpan data yang baru diimpor ke localStorage
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // BARU: Muat data dari localStorage SEBELUM melakukan hal lain
